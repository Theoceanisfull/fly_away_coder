...
RUN_BASELINE_PATH = EXPERIMENTS_ROOT / "run_baseline.py"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a plain baseline sweep across models and benchmarks.")
...
def prepare_sweep_root(args: argparse.Namespace) -> Path:
    if args.sweep_root:
        ...
        sweep_root = SWEEPS_ROOT / f"{time.strftime('%Y%m%dT%H%M%SZ', time.gmtime())}_baseline_matrix"
    ...
    if summary_payload["benchmark_aggregates"]:
        ...
        for row in summary_payload["benchmark_aggregates"]:
            lines.append(
                ...
                f"mean problem time `{row['mean_problem_time_s']:.2f}s`"
            )

...
RUN_POT_PATH = EXPERIMENTS_ROOT / "run_pot.py"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a ProgramOfThought sweep across models and benchmarks.")
    ...
    parser.add_argument("--livecodebench-max-iters", type=int, default=3)
    ...
    parser.add_argument("--scicode-max-iters", type=int, default=4)
...
def prepare_sweep_root(args: argparse.Namespace) -> Path:
    if args.sweep_root:
        ...
        sweep_root = SWEEPS_ROOT / f"{time.strftime('%Y%m%dT%H%M%SZ', time.gmtime())}_pot_matrix"
...
def success_summary_row(run_record: dict[str, Any], summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "avg_hops": summary["hop_stats"]["mean"],
        "max_hops": summary["hop_stats"]["max"],
    }
        ...
        aggregates.append(
            {
                ...
                "mean_hops": average([float(item["avg_hops"]) for item in items]),
            }
        )
    return aggregates

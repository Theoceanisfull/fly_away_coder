...
- lightweight aggregate reports in `benchmarks/baselines/`
- documentation explaining how a run was produced and what target it is trying to beat

## Updating Baselines

After a new full run finishes, copy only the report outputs into a new timestamped folder under `benchmarks/baselines/<benchmark>/<run_id>/`, then update this README and `manifest.json`.
...
Do not copy raw benchmark data or generation logs into `benchmarks/baselines/` unless there is a deliberate review decision to publish them.
